...
public class venEscudo extends JDialog implements ActionListener {
	/**
	 * Aqui definimos todos los buttons panels y combobox que hemos utilizado y el
	 * objgestor
	 */
	 ...
	 * Runnable() { public void run() { try { venEscudo frame = new venEscudo();
	...
	public venEscudo(venEquipacion objequipacion, boolean modal) {
		...
		comboForma.setModel(
				new DefaultComboBoxModel(new String[] { "Circulo", "Triangulo", "Heptagono", "Octagono", "Hexagono" }));
		...
		comboColor.setModel(new DefaultComboBoxModel(new String[] { "Amarillo", "Naranja", "Rojo", "Morado", "Azul",
	...
	/**
	 * Aqui a traves del switch iremos a las diferentes opciones que hay
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		case ACTION_COMMAND_SIGUIENTE:
			if (!comboForma.getSelectedItem().equals("") || comboColor.getSelectedItem().equals("")) {
				try {
					gestor.anadirEscudo((String) comboForma.getSelectedItem(), (String) comboColor.getSelectedItem());
				} catch (Exception exv) {
